...
T MessageQueue<T>::receive()
{
    ...
    auto firstMsg = std::move(_queue.front());
    _queue.pop_front();
...
void TrafficLight::waitForGreen() {
  ...
  // returns.
  while (TrafficLightPhase::green != _msgQueue.receive());
}
...
void TrafficLight::cycleThroughPhases() {
  ...
  int cycleDuration = 4000 + rand() % 2000; // duration of a single simulation cycle in ms
  ...
  while (true) {
    ...
    { 
      std::cout << "Check point. Thread ID #" << std::this_thread::get_id() << std::endl;
